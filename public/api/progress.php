...
try {
    ...
    switch ($action) {
        ...
        case 'update':
            ...
            $stmt = $conn->prepare("SELECT id FROM progress_belajar WHERE user_id = ? AND materi_id = ?");
            $stmt->execute([$user_id, $materi_id]);
            $existing = $stmt->fetch();
            
            if ($existing) {
                $stmt = $conn->prepare("UPDATE progress_belajar SET status = ?, persentase = ?, waktu_belajar = waktu_belajar + ?, updated_at = NOW() WHERE id = ?");
                $stmt->execute([
                    $input['status'] ?? 'sedang_belajar',
                    $input['persentase'] ?? 0,
                    $input['waktu_tambahan'] ?? 0,
                    $existing['id']
                ]);
            } else {
